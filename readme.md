...
```
tanzu apps workload create tap-spring-petclinic -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/tap-spring-petclinic-source \
  --type web
```
...
```
tanzu apps workload get tap-spring-petclinic
```
